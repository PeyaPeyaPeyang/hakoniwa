...
public class HakoniwaAccessManager {
    ...
    public static void checkMethodInvocation(
            ...
            String descriptor,
            Object[] arguments) {
        checkMethodInvocation(layerId, owner, name, descriptor, InvocationType.INVOKE_VIRTUAL.name(), arguments);
    }
    ...
    public static void checkMethodInvocation(
        ...
        MethodAction action = new MethodAction.Invocation(
                classMethod,
                operationOf(InvocationType.valueOf(invocationType)),
                arguments);
        check(layer, action);
    }

    private static MethodAction.MethodOperation operationOf(InvocationType invocationType) {
        if (invocationType == InvocationType.INVOKE_STATIC) {
            return MethodAction.MethodOperation.INVOKE_STATIC;
        }
        if (invocationType == InvocationType.INVOKE_DYNAMIC) {
            return MethodAction.MethodOperation.OTHER;
        }
        return MethodAction.MethodOperation.INVOKE_INSTANCE;
    }
